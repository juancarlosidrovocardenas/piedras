...
//JHtml::script('modules/'.$module->module.'/assets/js/jsmart.cookie.js');
...
$uniqueid = 'sjk2ajaxtabs_'.rand().time();
...
if( $list != null ){
	...
	foreach ($list as $first_category){
		$category_preload = $first_category[0]['cat_id'];
		break;
	}
	...
	} else if( (int)($catfirstload[0]) && isset($list[ (int)($catfirstload[0])])) {
		$category_preload = (int)($catfirstload[0]);
	} */
	
	$catfirstload = $params->get('category_preload');
	
	if( (int)($catfirstload[0]) && isset($list[ (int)($catfirstload[0])])) {
		$category_preload = (int)($catfirstload[0]);
	}
	...
	<!--[if lt IE 9]><div class="sj-k2-ajax-tabs <?php echo $css_posiotion?> msie lt-ie9" id="<?php echo $uniqueid; ?>" ><![endif]--> 
	<!--[if IE 9]><div class="sj-k2-ajax-tabs <?php echo $css_posiotion?> msie" id="<?php echo $uniqueid; ?>" ><![endif]-->
	<!--[if gt IE 9]><!--><div class="sj-k2-ajax-tabs <?php echo $css_posiotion?>" id="<?php echo $uniqueid; ?>" ><!--<![endif]-->
	<?php
		...
		<div class="tabs-content" <?php //echo $tabs_content_style; ?>>
			...
			foreach($list as $category):
				...
				$is_preload = $category[0]['cat_id'] == $category_preload;
				$css_selected = $is_preload ? "tab-content  category-id-{$category[0]['cat_id']} selected" : "tab-content category-id-{$category[0]['cat_id']}"; ?>
				...
				$container.tabsmart({
					...
					ajaxUpdate: function(element, options){
						...
						if ( !loaded ){
							...
							ajax_options = {
								sj_k2category_id:	category_id,
								sj_module_id:	options.sj_module_id,
								sj_module:		options.sj_module,
								sj_module_k2ajax_request: 1
							};
							$.ajax({
								type: 'POST',
								...
								/* 	var cookie_name = ajax_options.sj_module + '_' + ajax_options.sj_module_id;
									var cookie_value = ajax_options.sj_k2category_id;
...
<?php } else { echo JText::_('NO_CONTENT'); }?>