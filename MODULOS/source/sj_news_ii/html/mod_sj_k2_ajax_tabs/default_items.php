...
if ($nb_items>0){
	...
	if ((int)$params->get('pager_display', 1)){
		...
		<div class="pager-container">
			<ul class="pages">
				<!-- li><div class="page page-previous" data-target="#<?php echo $item_container; ?>" data-jslide="prev">&laquo; </div></li -->
			<?php for ($j=0; $j<$nb_pages; $j++) {?>
				<li><div class="page number-page <?php if( $j == 0 ){echo ' sel';}?>" data-target="#<?php echo $item_container; ?>" data-jslide="<?php echo $j;?>"><?php echo ($j+1); ?></div></li>
			<?php } ?>
				<!-- li><div class="page page-next" data-target="#<?php echo $item_container; ?>" data-jslide="next">&raquo;</div></li -->
			</ul>
		</div>
		<?php
	...
	<div class="items-container slide" id="<?php echo $item_container; ?>" data-interval="0">
		<div class="items-container-inner" >
			...
			$k = 0; foreach ($category_items as $key=> $item) {
				$i++;
...
<?php 		 
	...
	<div class="noitem"><?php echo JText::_('NO_ITEMS'); ?></div>
<?php }?>
