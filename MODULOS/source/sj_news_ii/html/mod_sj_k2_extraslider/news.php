  ...
  if(!empty($list)){
	...
	<!--[if gt IE 9]><!--><div id="<?php echo $tag_id;?>" class="sj-extraslider <?php if( $options->effect == 'slide' ){ echo $options->effect;}?>  <?php echo $class_respl; ?>"   data-interval="<?php echo $interval; ?>" data-pause="<?php echo $params->get('pause_hover'); ?>"><!--<![endif]-->
        ...
        <?php if($options->title_slider_display == 1){?>
            <div class="heading-title"><h3><?php echo $options->title_slider;?></h3></div><!--end heading-title-->
        <?php }?>		    
		...
		for($i=0; $i<$pags; $i++){ 
			...
			?>
				...
				<div class="item <?php if($i==1){echo "active";}?>">
					<div class="line">
				<?php }?>
					<div class="item-wrap <?php echo $options->theme;?> <?php if(($count%$item_of_page == 0 || $count== $count_item)) echo 'last';?>">
						<div class="item-wrap-inner">
							...
							<?php if( $options->item_title_display == 1 ){?>
								...
								<div class="item-author"><?php echo JText::_('K2_AUTHOR_TEMPLATE'),' ' ?><span><?php echo $item->author; ?></span></div>
								<div class="item-date"><?php  echo  JHTML::_('date', $item->created,JText::_('DATE_FORMAT_TEMPLATE'));?></div>
									...
									<?php else: ?>
										<?php if(isset($item->numOfComments) > 0): ?>
											<?php echo JText::_('- '); ?> <i class="fa fa-comments-o"></i><a href="<?php echo $item->link; ?>#itemCommentsAnchor">
											<?php echo $item->numOfComments; ?> <?php echo ($item->numOfComments>1); ?>
										</a>
										<?php else: ?>
											<?php echo JText::_('- '); ?> <a href="<?php echo $item->link; ?>#itemCommentsAnchor">
											<i class="fa fa-comments-o"></i><?php echo JText::_('0'); ?>
										</a>
										<?php endif; ?>
									<?php endif; ?>
									</div>
